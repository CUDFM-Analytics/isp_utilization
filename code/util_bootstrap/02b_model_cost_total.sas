...
so I just keep them here for not then manually move them ? Ask carter if better way... 
after running 03_boot_analysis...;
libname out "&projRoot\data_boot_processed\cost_total";
...
MODEL ind_cost_total = time int int_imp season1 season2 season3 
...
* cost model - UPDATE DV in WHERE statement, UPDATE MODEL statements;
ODS SELECT NONE; OPTIONS NONOTES;
...
CLASS bootunit 
      ...
      race(ref='non-Hispanic White/Caucasian') 
      sex(ref='Female') ;
MODEL cost_total = time int int_imp season1 season2 season3 
                   bh_oth16 bh_oth17 bh_oth18 bh_er16 bh_er17 bh_er18 bh_hosp16 bh_hosp17 bh_hosp18
                   adj_pd_total_16cat adj_pd_total_17cat adj_pd_total_18cat
