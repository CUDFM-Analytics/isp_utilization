...
%INCLUDE "S:/FHPC/DATA/HCPF_Data_files_SECURE/Kim/isp/isp_hcpf_202303/code/hcpf_00_config.sas";
...
PROC TRANSPOSE DATA = attr_all_pcmp0 
                OUT = out.attr_all_pcmp (drop=_name_);
BY  flag; 
ID  month; 
VAR n_members;
run; *;
...
PROC TRANSPOSE DATA = out.attr_rel_mar20
                OUT = out.attr_rel_mar20_t (drop=_name_);
BY  flag; 
ID  month; 
...
proc print data = out.attr_all_pcmp;
run;
...
proc print data = out.attr_rel_mar20_t; run;  
...
proc sgplot data = out.attr_rel_mar20;
yaxis label = " ";
styleattrs datacontrastcolors =(purple steel); 
...
proc print data = out.attr_rel_mar20; 
