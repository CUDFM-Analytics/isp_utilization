...
p "Root: &root";
p "Script: &script";
p "using macro macro_cost_genmod.sas";
p "Log File: &log";
p "Results File: &pdf";
RUN; 

/*SECTION 02: PROB MODEL*/
PROC GENMOD DATA  = data.analysis;
CLASS mcaid_id     
...
* OUT:[P_INTGROUP]      IN :[out.&dv._logit]
 prob of util------------------------------------------------ ;
proc plm restore=logit;
   score data=intgroup out=p_intgroup predicted=p_prob / ilink;
run;

* OUT:[CP_INTGROUP]     IN :[out.&dv_gamma]
prob of cost -------------------------------------------------;
proc plm restore=gamma;
