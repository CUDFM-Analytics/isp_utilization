...
%macro hurdle_genmod(dat=,pvar=,cvar=,dv=);
...
store out=out.costpc_logit_genmod;
run;
...
store out=out.costpc_gamma_genmod;
RUN;
TITLE; 
...
proc sql;
create table out.&dv._avp_genmod as
  ...
  from out.&dv._meanCost_genmod;
quit;

TITLE "&dv._avp_genmod"; 
proc print data = out.&dv._avp_genmod;
run;

proc means data = out.&dv._meancost_genmod;
by exposed;
