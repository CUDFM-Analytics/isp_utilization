...
VERSION  : 2023-10-18 Get updated datasets, up to and including June 2023 (less than July 01, 2023)
            - 10-18 Details: Updated rerun.time_dim to include q's through 16
...
LIBNAME rerun "S:/FHPC/DATA/HCPF_DATA_files_SECURE/Kim/isp/isp_utilization/data2";
...
DATA rerun.age_qrtr (DROP=month_qrtr RENAME=(month=qm2));
SET  rerun.time_dim (KEEP=month FY time month_qrtr WHERE=(month_qrtr= 2) ); 
format month date9.; 
RUN; 

PROC CONTENTS DATA = ana.qry_demographics VARNUM; RUN; 

* Let's try getting ages first....; 
DATA rerun.demo;
SET  ana.qry_demographics (keep=mcaid_id dob gender race);
do   time = 1 to 16; OUTPUT; 
END; 
RUN; 

PROC SORT DATA = rerun.demo; BY time mcaid_id; RUN;

PROC SQL; 
CREATE TABLE rerun.age1 AS 
select a.mcaid_id
     , a.dob
     , a.time
     , b.qm2
     , b.fy
FROM rerun.demo          AS A
LEFT JOIN rerun.age_qrtr AS B ON a.time=b.time; 
QUIT; 
...
SET  rerun.age1; 
age=%age(qm2, dob);
IF 0 <= age < 65 then keep=1; else keep=0;
run; 

PROC SORT DATA = rerun.age2; BY mcaid_id time; run; 
...
===========================================================================================;
DATA   rerun.qrylong_00 (DROP=managedCare WHERE=(pcmp_loc_id ne .));
LENGTH mcaid_id $11; 
...
DATA rerun.qrylong_00a (DROP=pcmp_loc_type_cd pcmp_type); 
SET  rerun.qrylong_00; 
pcmp_type = input(pcmp_loc_type_cd, best12.);
IF   pcmp_type in (32 45 61 62) then fqhc = 1 ; else fqhc = 0 ;
RUN; 

* [rerun.qrylong_01]======================================================================
Joins ana.qry_demographics, rerun.age_dim, and rae_dim
Purpose:
...
===========================================================================================;
PROC SQL; 
CREATE TABLE rerun.qrylong_01 AS
SELECT a.mcaid_id
     , a.month
     , a.dt_qrtr
     , a.FY
     ...
     , d.time
     , d.fy_qrtr
FROM rerun.qrylong_00a            AS A 
LEFT JOIN ana.qry_demographics  AS B ON a.mcaid_id = b.mcaid_id 
LEFT JOIN rerun.rae_dim           AS C ON a.enr_cnty = c.hcpf_county_code_c
...
PROC SQL; 
CREATE TABLE rerun.pcmp_dim AS 
SELECT distinct pcmp_loc_id
     , fqhc
FROM rerun.qrylong_01
...
===========================================================================================;
PROC SQL;
CREATE TABLE rerun.final_00 AS 
SELECT a.mcaid_id
     , a.month
     , a.dt_qrtr
     , a.time
     ...
     , a.race
     , b.age_cat
FROM rerun.qrylong_01    AS A 
INNER JOIN rerun.age_dim AS B ON (a.mcaid_id=b.mcaid_id AND a.time=b.time)
...
===========================================================================================;
PROC SQL;
CREATE TABLE rerun.qrylong_02 AS 
SELECT mcaid_id
     , time
     , FY
     , dt_qrtr
     , month
FROM rerun.qrylong_01
WHERE mcaid_id IN (SELECT mcaid_id FROM rerun.final_00);
QUIT; 

* [rerun.final_00] & [rerun.DEMO_1922] =========================================================
Subset to mcaid_id's that have an rae_assigned in FY's 19-22
===========================================================================================;
DATA rerun.final_01  (KEEP = mcaid_id month dt_qrtr FY time age_cat)
     rerun.demo      (KEEP = mcaid_id month dt_qrtr FY time sex race rae_person_new pcmp_loc_id budget_group);
SET  rerun.final_00 ; 
RUN;  
PROC SORT DATA=rerun.final_01; BY MCAID_ID FY TIME; RUN; 

* %INCLUDE ==============================================================================
Creates table with max months' pcmp. In case of ties, takes most recent 
1. MACRO for other demo vars
2. output: rerun.pcmp_attr_qrtr
===========================================================================================;
%LET ds = rerun.demo;
%INCLUDE "&util/code/util_dataset_pre/incl_extract_check_fy2023.sas"; * creates rerun.pcmp_attr_qrtr too; 
...
%concat_id_time(ds=rerun.final_01);
%concat_id_time(ds=rerun.age_dim);

PROC SQL ; 
CREATE TABLE rerun.final_02 AS 
SELECT a.mcaid_id
     , a.month
     , a.dt_qrtr
     , a.FY
...
LEFT JOIN rerun.pcmp_attr_qrtr         AS D   ON A.id_time_helper = D.id_time_helper
LEFT JOIN rerun.isp_un_pcmp_dtstart    AS F   ON D.pcmp_loc_id    = F.pcmp_loc_id    
LEFT JOIN race                       AS G   ON A.id_time_helper = G.id_time_helper
LEFT JOIN sex                        AS H   ON A.id_time_helper = H.id_time_helper;
QUIT ;  

DATA  rerun.final_03;
SET   rerun.final_02   (DROP=time_start_isp month id_time_helper);
RUN;
...
PROC SQL;
CREATE TABLE rerun.util_1 as
SELECT a.*
     , (a.pd_amt/b.index_2021_1) AS adj_pd_amount 
FROM   rerun.util_0      AS A
LEFT JOIN rerun.adj      AS b    ON a.dt_qrtr=b.date
WHERE mcaid_id IN (SELECT mcaid_id FROM rerun.final_03);
quit; 

PROC SQL;
CREATE TABLE rerun.util_2 AS
SELECT MCAID_ID
      , FY
      , dt_qrtr
      , month
...
%nodupkey(ds=rerun.util_2, out=rerun.util); *10/22: 33227472 // 6/7 28628763, 12; 
PROC PRINT DATA = rerun.util (obs=200); run; 
...
%create_qrtr(data=rerun.bh, set=rerun.bh_0, var = dt_qrtr, qrtr=time);

**RUN / exec 02_get_prep_telehealth to get rerun.tel here; 
PROC SORT data = rerun.tel; by mcaid_id month ; run; 

* rerun.QRYLONG_04 ==============================================================================
join bh and util to qrylong to get averages (all utils - monthly, bho, telehealth) to qrylong4
===========================================================================================;
PROC SQL; 
CREATE TABLE rerun.qrylong_03 AS 
...
FROM rerun.qrylong_02  AS A
LEFT JOIN rerun.bh     AS B ON a.mcaid_id=B.mcaid_id AND a.month=B.month
LEFT JOIN rerun.util   AS C ON a.mcaid_id=C.mcaid_id AND a.month=C.month
LEFT JOIN rerun.tel    AS D ON a.mcaid_id=D.mcaid_id AND a.month=D.month;
QUIT;  

* [rerun.qrylong_pre] ==============================================================================
===========================================================================================;
DATA rerun.qrylong_pre_0 rerun.qrylong_post_0 (DROP=bho_n_hosp bho_n_other); 
SET  rerun.qrylong_03;
IF month <  '01Jul2019'd THEN OUTPUT rerun.qrylong_pre_0;
IF month >= '01JUL2019'd THEN OUTPUT rerun.qrylong_post_0;
RUN;  

PROC SQL;
CREATE TABLE rerun.qrylong_pre_1 as
SELECT mcaid_id
...
FROM rerun.qrylong_pre_0
GROUP BY mcaid_id;
QUIT; 

* change adj to if elig = 0, then adj var = -1 and set bh variables to 0 where .; 
DATA rerun.qrylong_pre_2;
SET  rerun.qrylong_pre_1;
...
proc univariate noprint data=rerun.qrylong_pre_2; 
where &var gt 0; 
var &var; 
...
data rerun.pctl1719; merge pd17pctle pd18pctle pd19pctle ; run;

PROC PRINT DATA = rerun.pctl1719; RUN; 
...
%insert_pctile(ds_in = rerun.qrylong_pre_2, ds_out = adj0,             year = 17);
%insert_pctile(ds_in = adj0,              ds_out = adj1,             year = 18);
%insert_pctile(ds_in = adj1,              ds_out = rerun.qrylong_1719, year = 19);

* [rerun.final_04] ==============================================================================
Add FY1719 outcomes to final_03
===========================================================================================;
PROC SQL;
CREATE TABLE rerun.final_04 AS 
SELECT a.*
     , b.*
FROM rerun.final_03           AS A
LEFT JOIN rerun.qrylong_1719  AS B ON a.mcaid_id=b.mcaid_id;
QUIT; 
%nodupkey(ds = rerun.final_04, out=rerun.final_04); 
...
========================================================================================;
DATA rerun.qrylong_post_1 (KEEP = mcaid_id month time FY n_ed n_pc n_ffsbh n_tele adj_total adj_pc adj_rx);
SET  rerun.qrylong_post_0 (KEEP = mcaid_id    month   dt_qrtr     time       FY
...
DATA rerun.qrylong_post_2 (DROP=i);
SET  rerun.qrylong_post_1; 
...
PROC SQL;
CREATE TABLE rerun.qrylong_post_3 as
SELECT mcaid_id
     , count(*) as n_months
     , time
     , FY
...
FROM rerun.qrylong_post_2
GROUP BY mcaid_id, time;
QUIT; 

%nodupkey(ds=rerun.qrylong_post_3, out=rerun.qrylong_2023); 
* IT's OK THAT ITs HIGHER bc didn't subset bh, tele to memlist yet!!!; 

* [rerun.FINAL_05];
PROC SQL; 
CREATE TABLE rerun.final_05 AS 
SELECT a.*
     , b.*
     , c.fqhc
FROM rerun.final_04            AS A
LEFT JOIN rerun.qrylong_2023   AS B ON a.mcaid_id=b.mcaid_id AND a.time=b.time
LEFT JOIN rerun.pcmp_dim       AS C ON a.pcmp_loc_id=c.pcmp_loc_id;
QUIT; 
...
DATA rerun.final_06 (DROP = dt_qrtr elig: adj_pd_17pm adj_pd_18pm adj_pd_19pm);
...
PROC UNIVARIATE DATA = rerun.final_06;
BY FY; 
WHERE &VAR gt 0; 
VAR   &VAR;
...
PROC SORT DATA = rerun.final_06; BY FY; run; 

%pctl_2023(var = adj_total_pmpq,   out = rerun.adj_total_pctl,   pctlpre = adj_total_,  t_var = adj_total_95); 
%pctl_2023(var = adj_pc_pmpq,      out = rerun.adj_pc_pctl,      pctlpre = adj_pc_,     t_var = adj_pc_95); 
%pctl_2023(var = adj_rx_pmpq,      out = rerun.adj_rx_pctl,      pctlpre = adj_rx_,     t_var = adj_rx_95); 

data rerun.pctl2023; merge rerun.adj_total_pctl_a rerun.adj_pc_pctl_a rerun.adj_rx_pctl_a ; run;

PROC PRINT DATA = rerun.pctl2023; RUN; 
PROC PRINT DATA = rerun.adj_total_pctl; 
PROC PRINT DATA = rerun.adj_pc_pctl;
PROC PRINT DATA = rerun.adj_rx_pctl; RUN; 
...
PROC UNIVARIATE NOPRINT DATA = rerun.final_06; 
WHERE FY=&FY 
AND   &VAR gt &gt;
VAR   &VAR;
...
merge mu_total_20       mu_total_21     mu_total_22     mu_total_23
      ...
      rerun.adj_total_pctl_a  rerun.adj_pc_pctl_a   rerun.adj_rx_pctl_a;
RUN; 
PROC PRINT DATA = rerun.mu_pctl_2023; RUN; 
...
RUN; 
PROC SORT DATA = rerun.final_07; by mcaid_id time; run; 
...
DATA rerun.final_08 ;
SET  rerun.final_07 (DROP = adj_total_pmpq adj_pc_pmpq adj_rx_pmpq
...
PROC SORT DATA = rerun.final_08; BY mcaid_id time; RUN; 

* DATA.ANALYSIS ===========================================================================
===========================================================================================;
%INCLUDE "S:/FHPC/DATA/HCPF_DATA_files_SECURE/Kim/isp/isp_utilization/code/config_formats.sas"; 

* Numeric vars that can be set to length=3 to reduce dataset in rerun.final_08 step ; 
