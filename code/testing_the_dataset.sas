...
VERSION  : 2023-10-23 starting over?? Trying it... 
...
PROC FREQ DATA = tmp.qrylong_00; TABLES pcmp_loc_type_cd; where pcmp_loc_type_cd in ("32","45","61","62"); RUN; * should get 32216324 below;

DATA tmp.qrylong_00a (DROP=pcmp_loc_type_cd pcmp_type); 
SET  tmp.qrylong_00; 
pcmp_type = input(pcmp_loc_type_cd, best12.);
IF   pcmp_type in (32, 45, 61, 62) then fqhc = 1 ; else fqhc = 0 ;
RUN; 
PROC FREQ DATA = tmp.qrylong_00a; tables fqhc; run; * matches 32216324; 
...
%concat_id_time(ds=tmp.qrylong_01); 
...
INNER JOIN tmp.age_dim AS B ON (a.id_time_helper=b.id_time_helper)
...
SELECT mcaid_id
     ...
     , month
FROM tmp.qrylong_01
WHERE id_time_helper IN (SELECT id_time_helper FROM tmp.final_00);
QUIT; 

%nodupkey(ds=tmp.qrylong_02, out=tmp.qrylong_02);

PROC SQL; CREATE TABLE diff AS 
SELECT *
FROM   tmp.qrylong_02
where not exists (select * from tmp.final_01 where id_time_helper=id_time_helper); 
QUIT; 

* [int.final_00] & [RAW.DEMO_1922] =========================================================
Subset to mcaid_id's that have an rae_assigned in FY's 19-22
===========================================================================================;
DATA tmp.final_01  (KEEP = mcaid_id month dt_qrtr FY time age_cat id_time_helper)
     tmp.demo      (KEEP = mcaid_id month dt_qrtr FY time sex race rae_person_new pcmp_loc_id budget_group);
SET  tmp.final_00 ; 
RUN;  
...
PROC SQL;
CREATE TABLE tmp.util_01 as
SELECT a.*
     , (a.pd_amt/b.index_2021_1) AS adj_pd_amount 
FROM   tmp.util_01a    AS A
...
PROC PRINT DATA = tmp.util (obs=200); run; 
...
%create_qrtr(data=tmp.bh, set=raw.bh_0, var = dt_qrtr, qrtr=time);
...
QUIT; 
%nodupkey(ds = tmp.final_04, out=tmp.final_04); 
...
DATA tmp.qrylong_post_2 (DROP=i);
...
%nodupkey(ds=tmp.qrylong_post_3, out=tmp.qrylong_2023); 
...
LEFT JOIN tmp.qrylong_2023   AS B ON a.mcaid_id=b.mcaid_id AND a.time=b.time
...
PROC UNIVARIATE DATA = tmp.final_06;
BY FY; 
WHERE &VAR gt 0; 
VAR   &VAR;
...
PROC SORT DATA = tmp.final_06; BY FY; run; 
...
PROC UNIVARIATE NOPRINT DATA = tmp.final_06; 
WHERE FY=&FY 
AND   &VAR gt &gt;
VAR   &VAR;
...
DATA tmp.final_08 ;
SET  tmp.final_07 (DROP = adj_total_pmpq adj_pc_pmpq adj_rx_pmpq
...
PROC SORT DATA = tmp.final_08; BY mcaid_id time; RUN; 
