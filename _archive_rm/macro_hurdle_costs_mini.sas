...
***********************************************************************************************;
%macro hurdle_mini(dat=,pvar=,cvar=,dv=,type=);
...
REPEATED SUBJECT = mcaid_id / type = &type;
store mini.&dv._cmodel_&type;
output out = &dv._c_predout_&type
...
* OUT:[P_INTGROUP]      IN :[out=mini.&dv._pmodel]
 prob of util------------------------------------------------ ;
proc plm restore=mini.&dv._pmodel_&type ;
   score data=intgroup out=p_intgroup predicted=p_prob / ilink;
run;

* OUT:[CP_INTGROUP]     IN :[out=mini.&dv._cmodel]
prob of cost -------------------------------------------------;
proc plm restore=mini.&dv._cmodel_&type ;
   score data=p_intgroup out=cp_intgroup predicted=p_cost / ilink;
run;

* OUT:[mini.&dv._mean] IN:[cp_intgroup]
Person average cost is calculated------------------------------;
data mini.&dv._mean_&type ;
  set cp_intgroup;
  a_cost = p_prob*p_cost;* (1-p term = 0);
run;

* OUT:[mini.&dv._avp] IN:[mini.&dv._mean]
Group average cost is calculated and contrasted-----------------;
proc sql;
create table mini.&dv._avp_&type as
